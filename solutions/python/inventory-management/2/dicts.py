...
def add_inventory(inventory, item, amount):
    """"Add item to inventory.

    Parameters:
        inventory (dict): Dictionary of existing inventory.
        item (key): item in the list
        amount (int): the amount you need to sum

    Returns:
        dict: The inventory updated
    """
...
def create_inventory(items):
    ...
    inventory = {}
    for item in items:
        add_inventory(inventory, item, 1)
    return inventory
...
def add_items(inventory, items):
    ...
    for item, value in new_inventory.items():
        add_inventory(inventory, item, value)
    return inventory
